...
function do_debug_cmake_build {
  echo "Tried to ring bell. So you know to connect to cmake debugger."
  if [ -S "/tmp/pipe" ]; then
    echo "DELETING PIPE"
    rm -rf /tmp/pipe
  fi
  cmake --debugger --debugger-pipe=/tmp/pipe  \
    -GNinja \
    -DCMAKE_TOOLCHAIN_FILE="$SCRIPT_PATH/mingw64.cmake" \
    -DCMAKE_PREFIX_PATH="$INSTALL_BASE/lib/cmake;$INSTALL_BASE/lib;$INSTALL_BASE;/usr/include;/usr/lib/x86_64-linux-gnu/" \
    -DCMAKE_MODULE_PATH="$INSTALL_BASE/lib/cmake;$INSTALL_BASE/lib;$INSTALL_BASE" \
    -DCMAKE_INSTALL_PREFIX="$INSTALL_BASE" \
    -DBUILD_SHARED_LIBS=$ARG_SHARED \
    -DCMAKE_EXE_LINKER_FLAGS="$CMAKE_DEFAULT_FLAGS" \
...
function do_openssl_nmake_build {
  export CC=x86_64-w64-mingw32-gcc
  export AR=x86_64-w64-mingw32-ar
  export RANLIB=x86_64-w64-mingw32-ranlib
  export RC=x86_64-w64-mingw32-windres

  perl $2/Configure mingw64 no-tests enable-md4 \
    --prefix=$INSTALL_BASE    
  make -j$(nproc)
  make install
}
...
if [ $CLONE -ne 0 ]; then
  ...
  do_clone openssl-3.6.0 https://github.com/openssl/openssl.git openssl
fi
...
if [ $ARG_SHARED -eq 1 ]; then
  ...
  cp /usr/lib/gcc/x86_64-w64-mingw32/13-win32/libgcc_s_seh-1.dll $INSTALL_BASE/bin
  cp /usr/x86_64-w64-mingw32/lib/libwinpthread-1.dll $INSTALL_BASE/bin
fi