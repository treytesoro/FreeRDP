		...
		case SDL_PARENT_RESIZED:
			{
				...
				if (w != currentW || h != currentH) {
					...
					if(!p_sdlwindow->setChildWindowStyle(sdl_hwnd))
						SDL_Log("[ERROR][SdlWindow_SDLMessageHook] Could not set child windows style");
				}
				return false;
			}
			break;
...
[[nodiscard]] bool SdlWindow::setChildWindowStyle(HWND hwnd)
{
...
bool SdlWindow::setParent(Uint64 nativeWindowID)
{
	...
	{
		case sdl::utils::Windows:
			if(this->setParentWin32(nativeWindowID)) {
				return true;
			}
			break;
	}
...
bool SdlWindow::setParentWin32(Uint64 nativeWindowID) 
{
